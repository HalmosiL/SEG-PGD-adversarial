...
class DeepLabV3(nn.Module):
    ...
    def forward(self, x, y=None, indicate=0):
        ...
        #print(x.size(), self.zoom_factor, h, w)
        ...
        x_tmp = self.layer3(x)
        #print('layer3', x_tmp.shape)
        x = self.layer4(x_tmp)
        #print('layer4', x.shape)
        if self.use_aspp:
            x = self.aspp(x)
            #print('ppm output', x.shape)
        x = self.cls(x)
        #print('cls', x.shape)
...
class PSPNet(nn.Module):
    ...
    def forward(self, x, y=None, indicate=0, custom_loss=False, ML_loss=False):
        if ML_loss: 
            return self.forward_ML_loss(x, y, indicate)
        ...
        if self.training or indicate==1:
            ...
            main_loss = self.criterion(x, y)
            aux_loss = self.criterion(aux, y)
            if custom_loss: return x.max(1)[1], y, aux, features
            if indicate==1:
                return x.max(1)[1], main_loss, aux_loss, x
            else:
                return x.max(1)[1], main_loss, aux_loss, features
        else:
            return x

    def forward_ML_loss(self, x, y=None, indicate=0, custom_loss=False):
        x_size = x.size()
        assert (x_size[2]-1) % 8 == 0 and (x_size[3]-1) % 8 == 0
        h = int((x_size[2] - 1) / 8 * self.zoom_factor + 1)
        w = int((x_size[3] - 1) / 8 * self.zoom_factor + 1)
        ...
        x = self.layer2(x)
        x_tmp = self.layer3(x)
        x_pool = self.layer4(x_tmp)
        
        if self.use_ppm:
            features = self.ppm(x_pool)
        x = self.cls(features)
        if self.zoom_factor != 1:
            x = F.interpolate(x, size=(h, w), mode='bilinear', align_corners=True)

        return [x_pool, features], x



class PSPNet_2Heads(nn.Module):
    def __init__(self, layers=50, bins=(1, 2, 3, 6), dropout=0.1, classes=2, zoom_factor=8, use_ppm=True, criterion=nn.CrossEntropyLoss(ignore_index=255), BatchNorm=nn.BatchNorm2d, pretrained=True):
        super(PSPNet_2Heads, self).__init__()
        assert layers in [50, 101, 152]
        assert 2048 % len(bins) == 0
        assert classes > 1
        assert zoom_factor in [1, 2, 4, 8]
    ...
    def forward(self, x, y=None, indicate=0, adv=False):
        x_size = x.size()
        assert (x_size[2]-1) % 8 == 0 and (x_size[3]-1) % 8 == 0
        h = int((x_size[2] - 1) / 8 * self.zoom_factor + 1)
        w = int((x_size[3] - 1) / 8 * self.zoom_factor + 1)
